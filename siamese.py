...
class SiameseNetwork(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        return self.fc_out(combined).squeeze(-1)  # shape -> [batch_size]
...
def train(model, dataloader, criterion, optimizer, device):
    ...
    for x1, x2, y in dataloader:
        ...
        optimizer.zero_grad()
        outputs = model(x1, x2)  # [batch_size]
...
def load_pairs_and_labels(csv_file):
    ...
    with open(csv_file, "r") as f:
        ...
        for row in reader:
            # Take the two sequences as a tuple
            sample_pairs.append((row["on_seq"], row["off_seq"]))
            # Convert label to int/float as needed
